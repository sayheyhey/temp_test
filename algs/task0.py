...
def env_agent_config(cfg):
    ...
    # else:
    action_dim = env.action_space.n  # 动作维度，CartPole-v0中动作空间的维度是2
...
def train(cfg,env,agent):
    ...
    for i_ep in range(cfg.train_eps):
        ...
        while not done:
            ...
            if steps % cfg.update_fre == 0: # 策略更新频率
                agent.update()
            state = state_
...
if __name__ == "__main__":
    ...
    '''
    env,agent的作用：
    env:通过过env获取state_dim,action_dim
    agent:由上述的dim生成智能体，包括choose_action,update
    '''
